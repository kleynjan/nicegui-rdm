...
store.set_derived_fields(derived_fields: dict[str, Callable], dependencies=None, query_map=None)
```

Derived fields are computed *after* the read, so they cannot be queried: passing a derived name to `filter_by`, `order_by` or `group_by` raises a `ValueError`. Point `Column.sort_key` at a real field instead — or give the name real fields to stand in for it:

```python
store.set_derived_fields(
    {"member_name": lambda r: f"{r['member__first_name']} {r['member__last_name']}"},
    dependencies=["member__first_name", "member__last_name"],
    query_map={"member_name": ["member__first_name", "member__last_name"]},
)
```

`order_by` then uses the **first** mapped field and `search_q` ORs over **all** of them, which makes a derived column both sortable and searchable. A derived name that reaches the DB inside a `q=` predicate cannot be caught up front; `TortoiseStore` re-raises the resulting `FieldError` as a `ValueError` naming the store's derived fields.

**Predicate building (used by `TableConfig(show_search=True)`):**

```python
store.search_q(text: str, fields: list[str]) -> Any | None   # OR of case-insensitive matches
store.and_q(a, b) -> Any | None                              # compose; None = no constraint
```

`TortoiseStore` returns a Tortoise `Q`; `DictStore` returns a callable predicate; the base `Store` returns `None`. Tables never build predicates themselves — that is what keeps them free of ORM knowledge and makes search testable on `DictStore`. Both methods are part of the `RdmDataSource` protocol.

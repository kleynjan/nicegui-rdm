...
**Custom table subclasses** — subclass `ObservableRdmTable` directly to build bespoke table behaviour (e.g. inline per-cell editing). Implement `@ui.refreshable_method async def build(self)` rendering headers and rows only (the inherited `render()` places the toolbars around it), call `self.load_data()`, and use `self._render_cell(col, value, row)` for read-only cells. For editable cells, use `build_cell_field(col, row_dict)` (from `ng_rdm.components.fields`) — it produces a label-less, compact widget correctly styled for table cells. See `examples/in_row_editing.py` for a full working example.
...
```python
# Toolbars + table — the normal entry point, called once
await table.render()

# Bare table, no toolbar at all:
await table.build()

# build() is the refreshable: build.refresh() re-renders headers and rows,
# leaving toolbar content (and its focus/value) untouched.
```
